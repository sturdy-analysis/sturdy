...
/*
 * We now implement a concrete interpreter for the while language. To this end we need to provide concrete
 * implementations for all value and effect interfaces (this step can be skipped if reusing existing components
 * from the Sturdy library) and instantiate the generic interpreter with these concrete implementations.
 */
...
/*
 * The concrete interpreter can be derived from the generic interpreter by instantiating it with the
 * appropriate components.
 */
class ConcreteInterpreter extends GenericInterpreterFirstShot[Int, NoJoin]:
