...
//   private var _freedReferences: Map[ApronVar.UID, ApronExpr[Addr]] = Map()

//   private def freeReference(v: ApronVar, e: ApronExpr[Addr]): Unit =
//     if (Apron.debugAlloc)
//       println(s"Free reference $v = $e")
//     _freedReferences += v.uid -> e
//   private inline def getFreedReferences: Map[ApronVar.UID, ApronExpr[Addr]] = _freedReferences
...
//         else
//           ApronExpr[Addr].topInterval
//     override def getBound(e: ApronExpr[Addr]): Interval =
//       apronState.getBound(apronManager, e.toIntern(this, allowOpen = false))
//     override def getFreedReference(v: ApronVar): Option[ApronExpr[Addr]] =
...
//       freeReference(v, ApronExpr[Addr].Constant(oldVal))
//       if (env.hasVar(v.av)) {
...
//   def assign(v: alloc.Var, exp: ApronExpr[Addr]): Option[alloc.Var] =
//     if (isFreed(v))
...
//       val freedJoined = JoinMap[ApronVar.UID, ApronExpr[Addr]] (using (e1, e2) =>
...
//       val freedWidened = WidenFiniteKeyMap[ApronVar.UID, ApronExpr[Addr]](using (e1, e2) =>
//         val e1norm = e1.normalize(as1)
//         val e2norm = e2.normalize(as2)
...
//     private var fFreedReferences: Map[ApronVar.UID, ApronExpr[Addr]] = _
...
// class ApronState(val apronManager: apron.Manager, val cs: Abstract1, val freed: Map[ApronVar.UID, ApronExpr[Addr]]) extends ApronScope:
...
//   override def getBound(v: ApronExpr[Addr]): Interval =
...
//   def copy(newFreed: Map[ApronVar.UID, ApronExpr[Addr]]) =
//     new ApronState(apronManager, cs, newFreed)

//   def copy(newCs: Abstract1, newFreed: Map[ApronVar.UID, ApronExpr[Addr]]) =
