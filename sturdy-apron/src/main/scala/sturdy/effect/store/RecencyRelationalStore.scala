...
object RecencyRelationalStore:
  def apply[Ctx: Ordering: Finite, Type: ApronType: Join: Widen](apronManager: Manager):
      (RecencyStore[Ctx, PowVirtualAddress[Ctx], ApronExpr[PhysicalAddress[Ctx], Type]], RelationalStore[Ctx, Type, PowersetAddr[PhysicalAddress[Ctx], PhysicalAddress[Ctx]], ApronExpr[PhysicalAddress[Ctx], Type]]) =
    ...
    given CombineApronPhysExpr[W <: Widening]: Combine[ApronExprPhysAddr, W] = {
    ...
    val apronStore: RelationalStore[Ctx, Type, PowPhysAddr, ApronExprPhysAddr] = RelationalStore(
      apronManager,
