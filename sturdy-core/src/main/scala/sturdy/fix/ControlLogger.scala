...
class ControlLogger[Ctx, Dom, Codom, Node]
  ...
  private var predecessors: Set[CNode] = Set(null.asInstanceOf[CNode])
  ...
  private val nodes: mutable.Set[CNode] = mutable.Set()
  ...
  def logger(using contextual: Contextual[Ctx, Dom, Codom]): Logger[Dom, Codom] = new Logger {
    override def enter(dom: Dom): Unit = getDomNode(dom) match
      case Some(node) =>
        val cnode = CNode(node, null.asInstanceOf[Ctx])
        nodes += cnode
      ...
      codom match
        ...
        case Failure(RecurrentCall(frame)) =>
          ...
          if (frame.dom == dom/* && frame.ctx == ctx*/) {
            val corecurNode = CNode(getDomNode(dom.asInstanceOf[Dom]).getOrElse(throw new IllegalStateException(s"Correcurrent input $dom must be associated with a node")), null.asInstanceOf[Ctx])
  ...
  protected def nodeGraphVizAttributes(from: CNode): String = (from.node match
