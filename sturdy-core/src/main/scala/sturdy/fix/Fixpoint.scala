...
trait ContextualFixpoint[Dom, Codom] extends CombinatorFixpoint[Dom, Codom]:
  ...
  protected def contextSensitive: Contextual[Ctx, Dom] ?=> Combinator[Dom, Codom]

  private inline def withContext(phi: Contextual[Ctx, Dom] ?=> Combinator[Dom, Codom]): ContextSensitive[Ctx, Dom, Codom] =
  ...
  private val contextSensitiveLoggers: ListBuffer[Contextual[Ctx, Dom] ?=> Logger[Dom, Codom]] = ListBuffer()
  def addContextSensitiveLogger(logger: Contextual[Ctx, Dom] ?=> Logger[Dom, Codom]): Unit = contextSensitiveLoggers += logger
  def removeContextSensitiveLogger(logger: Contextual[Ctx, Dom] ?=> Logger[Dom, Codom]): Unit = contextSensitiveLoggers -= logger
...
trait ContextInsensitiveFixpoint[Dom, Codom] extends ContextualFixpoint[Dom, Codom]:
  ...
  final protected override def contextSensitive: Contextual[Ctx, Dom] ?=> Combinator[Dom, Codom] = contextInsensitive
  protected def contextInsensitive: Contextual[Ctx, Dom] ?=> Combinator[Dom, Codom]
...
object Fixpoint:
  ...
  val DEBUG_INVARIANTS: Boolean = System.getProperty("STURDY_DEBUG_INVARIANTS", "false").toBoolean
