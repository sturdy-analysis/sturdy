...
type IncompleteEdge = (Node, EdgeType)
...
class ControlEventGraphBuilder[Atom,Section,Exc] extends ControlObserver[Atom,Section,Exc]:
  ...
  var stack: mutable.Stack[List[IncompleteEdge]] = mutable.Stack.empty
  ...
  private def completeEdge(current : Node, edgeType: Option[EdgeType]) : Unit =
    val protoEdges : List[IncompleteEdge] = stack.pop()
    protoEdges.foreach { (prev, et : EdgeType) =>
      edges += ((prev, current, edgeType.getOrElse(et) ))
    }
