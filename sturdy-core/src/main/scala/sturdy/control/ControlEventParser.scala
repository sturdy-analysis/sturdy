...
class ControlEventParser[Atom,Section,Exc,Fx] extends ControlObserver[Atom,Section,Exc,Fx]:
  ...
  private def addTree(t: CT, st: List[Entry]): List[Entry] = st match
    ...
    case Entry.Handler(exc, t0) :: st_ => Entry.Handler(exc, CT.Seq(t0, t)) :: st_
    ...
    ev match
      ...
      case BeginHandle(exc: Exc) => stack match
        case Entry.Catching(body, hs) :: stack_ =>
          stack = Entry.Handler(exc, CT.Empty()) :: Entry.Catching(body, hs) :: stack_
        case _ => error(s"Entry mismatch, expected Catching for $ev: $stack")
      case EndHandle() => stack match
        case Entry.Handler(hx, t) :: Entry.Catching(body, hs) :: stack_ =>
  ...
  enum Entry:
    ...
    case Handler(exc: Exc, t: CT)
    case ForkFirst(t: CT)
    case ForkSecond(first: CT, t: CT)
    ...
    override def toString: String = this match
      ...
      case Handler(exc, t) => s"Handler($exc, ${t.toString.take(10)}...)"
