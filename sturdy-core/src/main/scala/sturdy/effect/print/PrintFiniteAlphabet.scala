...
class PrintFiniteAlphabet[A](using Finite[A]) extends Print[A], Monotone:
  ...
  def isSound[C](c: CPrint[C])(using s: Soundness[C, A]): IsSound = boundary:
    c.getPrinted.foreach { c =>
      val aproxOk = symbols.exists(a => s.isSound(c, a).isSound)
      if (!aproxOk)
        break(IsSound.NotSound(s"concrete printed $c symbol is not approximated by alphabet $symbols"))
    }
    IsSound.Sound