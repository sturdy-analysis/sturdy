...
class LiftedObjectOps[FieldIdentifier, ObjectIdentifier, Value, Class, ObjectValue, Site, Method, StaticMethodDeclaration, Boolean, InvokeContext, FieldAccessContext, J[_] <: MayJoin[_], UOV, UB]
(extractO: ObjectValue => UOV, injectO: UOV => ObjectValue, extractB: Boolean => UB, injectB: UB => Boolean)
(using ops: ObjectOps[FieldIdentifier, ObjectIdentifier, Value, Class, UOV, Site, Method, StaticMethodDeclaration, UB, InvokeContext, FieldAccessContext, J])
  extends ObjectOps[FieldIdentifier, ObjectIdentifier, Value, Class, ObjectValue, Site, Method, StaticMethodDeclaration, Boolean, InvokeContext, FieldAccessContext, J]:
  ...
  override def invokeMethod(context: InvokeContext)(staticMethod: StaticMethodDeclaration, obj: ObjectValue, args: Seq[Value])(invoke: (ObjectValue, Method, Seq[Value]) => Value): Value =
    def liftedInvoke = (uov: UOV, mth: Method, vs: Seq[Value]) => invoke(injectO(uov), mth, vs)

    ops.invokeMethod(context)(staticMethod, extractO(obj), args)(liftedInvoke)
