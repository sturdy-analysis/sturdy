...
//
//
//enum NInterval[I](using Numeric[I], Ordering[I]):
//  case NITop
//  case NI(low: I, high: I)
//
//  import NInterval.*
//
//  override def toString: String = this match
//    case NI(low, high) => s"[$low, $high]"
//    case NITop => "NITop"
//
//  def toHexString: String = this match
//    case NI(low, high) => s"[${java.lang.Long.toHexString(low.toLong)}, ${java.lang.Long.toHexString(high.toLong)}]"
//    case NITop => "NITop"
//
//  def containsNum(n: I): Boolean = this match
//    case NI(low, high) => low <= n && n <= high
//    case NITop => true
//
//  def toBoolean: Topped[Boolean] = this match
//    case NI(low, high) =>
//      val zero = summon[Numeric[I]].fromInt(0)
//      if (low == zero && low == high) Topped.Actual(false)
//      else if (low <= zero && zero <= high) Topped.Top
//      else Topped.Actual(true)
//    case NITop => Topped.Top
//
//  def map[J](f: I => J): NInterval[J] = this match
//    case NI(low, high) => NI(f(low), f(high))
//    case NITop => NITop
//
//  def isConstant: Boolean = this match
//    case NI(low, high) => low == high
//    case NITop => false
//
...
//
//  override def neg(v: NumericInterval[I]): NumericInterval[I] = ???
//
...
//  }
//
//  val top: NInterval[I] = NInterval.NITop
//
//
//  def integerLit(i: I): NumericInterval[I] = NInterval.NI(i, i)
//  def randomInteger(): NumericInterval[I] = top
//
//  def intervalToRange(x: NInterval[I]): ArrayBuffer[I] = {
//    var i = x.low
