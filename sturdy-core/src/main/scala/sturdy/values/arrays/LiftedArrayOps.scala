...
class LiftedArrayOps[AID, Idx, V, AV, AType, ArrayElemAllocSite, Context, J[_] <: MayJoin[_], UAV, UIdx]
  (extractA: AV => UAV, injectA: UAV => AV, extractIdx: Idx => UIdx, injectIdx: UIdx => Idx)
  (using ops: ArrayOps[AID, UIdx, V, UAV, AType, ArrayElemAllocSite, Context, J]) extends ArrayOps[AID, Idx, V, AV, AType, ArrayElemAllocSite, Context, J]:
  override def makeArray(aid: AID, valueSupplier: Int => (V, ArrayElemAllocSite), arrayType: AType, arraySize: Idx): AV = injectA(ops.makeArray(aid, valueSupplier, arrayType, extractIdx(arraySize)))
