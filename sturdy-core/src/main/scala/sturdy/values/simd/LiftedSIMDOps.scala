...
class LiftedSIMDOps[B, V, I](extract: V => I, inject: I => V)(using ops: SIMDOps[B, I, V, Byte]) extends SIMDOps[B, V, V, Byte] {
  ...
  override def vectorAdd(shape: LaneShape, overflow: Overflow, sign: BitSign, v1: V, v2: V): V = inject(ops.vectorAdd(shape, overflow, sign, extract(v1), extract(v2)))
  override def vectorSub(shape: LaneShape, overflow: Overflow, sign: BitSign, v1: V, v2: V): V = inject(ops.vectorSub(shape, overflow, sign, extract(v1), extract(v2)))
