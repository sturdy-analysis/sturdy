...
class RecencyAbstractionTest(emptyStore: => RecencyStore[Ctx, PowVAddr, NumericInterval[Int]]) extends AnyFunSuite:
  ...
  test("Recency store joins most recent address into old address upon new allocation") {
    ...
    store.write(PowVirtualAddress(a1), NumericInterval(1, 2))
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 2)))
    ...
    store.write(PowVirtualAddress(a2), NumericInterval(5, 6))
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 2)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(5, 6)))
    ...
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 6)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(1, 6)))

    store.write(PowVirtualAddress(a3), NumericInterval(8, 9))
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 6)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(1, 6)))
    store.read(PowVirtualAddress(a3)) should be(JOptionA.Some(NumericInterval(8, 9)))
  }
  ...
  test("Allocation of the same context in two different branches") {
    ...
    val effectStack: EffectStack = EffectStack(
      RecencyClosure(store)
    )

    val ctx1 = "ctx1"
    val a1 = store.alloc(ctx1)
    store.write(PowVirtualAddress(a1), NumericInterval(3,4))
    ...
    a3.physical shouldBe PowersetAddr(PhysicalAddress(ctx1, Recent))
    store.read(PowVirtualAddress(a3)) should be(JOptionA.Some(NumericInterval(1, 6)))
  }
  ...
  test("Allocate addresses for the same context in separate branches") {
    ...
    effectStack.joinComputations {
      a2 = store.alloc(ctx1)
      store.write(PowVirtualAddress(a2), NumericInterval(3, 4))
    } {
      a3 = store.alloc(ctx1)
      store.write(PowVirtualAddress(a3), NumericInterval(5, 6))
    }
    ...
    a3.physical shouldBe PowersetAddr(PhysicalAddress(ctx1, Recent))
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 2)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(3, 6)))
    store.read(PowVirtualAddress(a3)) should be(JOptionA.Some(NumericInterval(3, 6)))
  }
  ...
  test("Strong updates on the same address in separate branches") {
    ...
    effectStack.joinComputations(
      ...
      store.write(PowVirtualAddress(a1), NumericInterval(5, 6))
    )
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(1, 6)))
  }
  ...
  test("Failing Branch") {
    ...
    var a2: VirtualAddress[Ctx] = null
    var a3: VirtualAddress[Ctx] = null
    store.write(PowVirtualAddress(a1), NumericInterval(0, 0))
    effectStack.joinComputations {
      a2 = store.alloc(ctx1)
      store.write(PowVirtualAddress(a2), NumericInterval(1, 1))
    }{
      a3 = store.alloc(ctx1)
      store.write(PowVirtualAddress(a3), NumericInterval(2, 2))
    ...
    store.read(PowVirtualAddress(a1)) should be(JOptionA.Some(NumericInterval(0, 0)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(1, 1)))
    store.read(PowVirtualAddress(a3)) should be(JOptionA.None())
  }
  ...
  test("Reaching fixpoint") {
    ...
    val a1 = store.alloc(ctx1)
    val a2 = store.alloc(ctx1)
    store.write(PowVirtualAddress(a1), NumericInterval(1, 4))
    store.write(PowVirtualAddress(a2), NumericInterval(3, 4))
    val state1 = (store.getState, store.addressTranslation.getState)
    val a3 = store.alloc(ctx1)
  ...
  test("Example 1 in \"Revisiting Recency Abstraction for JavaScript\" with Addr = AllocSite x Recency") {
    ...
    store.write(PowVirtualAddress(a1), NumericInterval(1, 1))
    /* l1: */ effectStack.joinComputations {
      /* l2: */ store.write(PowVirtualAddress(a1), NumericInterval(2, 2))
    ...
    store.read(PowVirtualAddress(a1)) should beGreaterThan(JOptionA.Some(NumericInterval(2, 2)))
    store.read(PowVirtualAddress(a2)) should be(JOptionA.Some(NumericInterval(1, 1)))
  }
  ...
  test("Example 1 in \"Revisiting Recency Abstraction for JavaScript\" with Addr = Unit x Recency") {
    ...
    val ctx = "Unit"
    /* l0: */ a1 = store.alloc(ctx); store.write(PowVirtualAddress(a1), NumericInterval(1, 1))
    /* l1: */ effectStack.joinComputations {
      /* l2: */ store.write(PowVirtualAddress(a1), NumericInterval(2, 2))
      /* l3: */ a2 = store.alloc(ctx); store.write(PowVirtualAddress(a2), NumericInterval(1, 1))
    } {
    }
