...
class TestControlTreeGraphBuilder extends AnyFunSuite {
  ...
  type CT = ControlTree[String, String, String, String]
  type AtomPair = (ControlTree[String, String, String, String], Node[String, String])
  val controlTreeGraphBuilder: ControlTreeGraphBuilder[String, String, String, String] = ControlTreeGraphBuilder()
  private val nFailure: AtomPair = (Failed(), Node.Failure[String, String]())
  private val nStart: AtomPair = (null, Node.Start())

  inline def testGraph[A, S, E, F](name: String)(f: => (ControlTree[A, S, E, F], Set[Edge[A, S]])): Unit =
    test(name) {
      val graphBuilder = new ControlTreeGraphBuilder[A, S, E, F]
      val (ct, expected) = f
  ...
  private given scala.Conversion[AtomPair, ControlTree[String, String, String, String]] = _._1
  ...
  testGraph("Empty Try") {
    ...
    val secMain = createSection("secMain", a1 + Try(a2 + a3, List.empty) + a4)
  ...
  testGraph("Try with throw and handle") {
    ...
    val secMain = createSection("secMain", a1 + Try(a2 + Throw("Exc1"), List(("Exc1", a3))) + a4)
  ...
  testGraph("Try with may throw and handle") {
    ...
    val secMain = createSection("secMain", a1 + Try(a2 + Fork(Empty(), Throw("Exc1")), List(("Exc1", a3))) + a4)
  ...
  testGraph("Try with nested block, correct dispatch") {
    ...
    val tryInner = Try(a3 + Fork(Empty(), Throw("Exc1")), List(("Exc1", handleIn)))
    val tryOuter = Try(a2 + Fork(Empty(), Throw("Exc1")) + tryInner, List(("Exc1", handleOut)))
  ...
  testGraph("Try with nested rethrow, correct dispatch") {
    ...
    val tryInner = Try(Fork(a1 + Throw("Exc1"), a2 + Throw("Exc2")), List(("Exc1", handle + Throw("Exc2")), ("Exc2", handleIn)))
    val tryOuter = Try(tryInner + a3, List(("Exc2", handleOut)))
  ...
  testGraph("Try nested, handle outer") {
    ...
    val tryInner = Try(a3 + Throw("Exc1"), List.empty)
    val tryOuter = Try(a2 + tryInner, List(("Exc1", handle)))
  ...
  testGraph("Try with nested block, rethrow") {
    ...
    val tryInner = Try(a3 + Fork(Empty(), Throw("Exc1")), List(("Exc1", handleIn + Throw("Exc1"))))
    val tryOuter = Try(a2 + Fork(Empty(), Throw("Exc1")) + tryInner, List(("Exc1", handleOut)))
  ...
  testGraph("Try with multiple throw") {
    ...
    val secMain = createSection("secMain", Try(a1 + Fork(a2, Fork(Throw("Exc1"), a3 + Throw("Exc2"))), List(("Exc1", handle1), ("Exc2", handle2))) + a4)
  ...
  testGraph("Try with failure") {
    ...
    val secMain = createSection("secMain", a1 + Try(Failed(), List(("Exc1", a3))))
  ...
  testGraph("Try with failure in handler") {
    ...
    val secMain = createSection("secMain", a1 + Try(a2 + Fork(Empty(), Throw("Exc1")), List(("Exc1", Failed()))) + a4)
