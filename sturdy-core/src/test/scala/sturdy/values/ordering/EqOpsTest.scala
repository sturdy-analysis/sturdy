...
class EqOpsTest
  ...
  (
    specials: Seq[L],
    val makeEqOps: () => (IsInterval[L, V], IntervalEqOps[L,V, B])
  )
  (using
...
//  test("1 != 1") {
//    val (ivOps, eqOps) = makeEqOps()
//    val actual = eqOps.getBool(eqOps.neq(ivOps.constant(Numeric[L].fromInt(1)), ivOps.constant(Numeric[L].fromInt(1))))
...
//  }
  ...
  test("[1, 2] == [-1, 3]") {
    ...
    eqOps.getBool(
      eqOps.equ(
        ivOps.interval(Numeric[L].fromInt(1), Numeric[L].fromInt(2)),
        ivOps.interval(Numeric[L].fromInt(-1), Numeric[L].fromInt(3)),
      )
    )shouldBe Topped.Top
  }