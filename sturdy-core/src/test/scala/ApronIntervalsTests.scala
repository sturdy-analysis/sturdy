...
class ApronIntervalsTest extends AnyFreeSpec, Matchers:
  ...
  manager.setPreferedScalarType(1) // 2 - mpfr (multi-precision floating real); 1 - mpq (rational); 0 - double 
  ...
  def testApronIntervalJoin(
    ...
    val expectedResultInterval = apron.Interval(expectedResultLower, expectedResultHigher)
    ...
    val expectedResultAbstract0Domain = apron.Abstract0(manager, 1, 0, Array(expectedResultInterval))
