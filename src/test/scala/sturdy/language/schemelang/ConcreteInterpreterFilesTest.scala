...
import sturdy.language.schemelang.SchemeExprParser.*
...
class ConcreteInterpreterFilesTest extends AnyFlatSpec, Matchers:
  ...
  def run(f: String, debug: Boolean): Value =
    val uri = classOf[SchemeExprParserTest].getResource("/sturdy/language/scheme/"++f).toURI();
