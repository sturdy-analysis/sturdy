...
case class RecurrentCall(ix: Int) extends Exception {
  override def toString: String = s"RecurrentCall($ix)"
}
object RecurrentCall:
  def force[A](t: Try[A]): Try[A] = t match
    case Failure(ex: RecurrentCall) => throw ex
    case _ => t
 ...
 *   - Recurrent and original calls: When a call (identified by its context) is pushed while it is already on the stack,
 *       the newly pushed call is recurrent while the previously pushed call is original.
 *
...
final class Stack[Dom, Codom, In, Out, Ctx](state: AnalysisState[In, Out], context: ContextSensitive[Dom, In, Ctx])
  ...
  (using widenCodom: Widening[Codom], widenOut: Widening[Out], j: JoinComputation):
  ...
  /** Cache of the outputs of previously executed stack frames. */
  ...
  def push(dom: Dom, in: In): Option[Try[Codom]] =
    ...
    stack.get(ctx) match
      ...
      case Some(ix) =>
        // call is recurrent
        recurrentCalls += ix
        // store the input state so the original call can consider it
        storeInState(ctx, in)
  ...
  def pop(dom: Dom, in: In, result: Try[Codom]): Boolean =
    ...
    if (recurrentCalls.remove(newStackHeight)) {
      ...
      if (Fixpoint.DEBUG)
        println(("  " * stackHeight) + s"  POP  $ctx")
      false
    }

  @inline private def storeInState(ctx: Ctx, in: In): Unit =
    val previousIn = inCache(ctx)
  ...
  @inline private def loadOutputOfRecurrentCall(ctx: Ctx, ix: Int): Option[Try[Codom]] = outCache.get(ctx) match
    case None =>
      if (Fixpoint.DEBUG)
        println(("  " * stackHeight) + s"  ## RECURRENT $ctx (fresh)")
  ...
  @inline private def storeOutput(ctx: Ctx, result: Try[Codom]): Unit = outCache.get(ctx) match
    ...
    case Some((previousResult, previousOut)) =>
      ...
      // should we do this here?   `state.setOutState(joinedOut)`
...
object Stack:
  class StackBuilder[Dom, Codom, In, Out]
    ...
    (using joinIn: JoinValue[In], joinOut: JoinValue[Out])
    (using wCodom: Widening[Codom], wOut: Widening[Out], jComp: JoinComputation) {
    def apply[Ctx](context: ContextSensitive[Dom, In, Ctx]) = new Stack(state, context)(using joinIn, joinOut)(using wCodom, wOut, jComp)
  }
  def apply[Dom, Codom, In, Out]
    ...
    new StackBuilder(state)(using joinIn, joinOut)(using wCodom, wOut, jComp)