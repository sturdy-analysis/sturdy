...
object IntervalAnalysis:
  enum Value:
    ...
    case DoubleValue(d: DoubleInterval)
    ...
    def asDouble: DoubleInterval = this match
      case DoubleValue(d) => d
      case TopValue => DoubleInterval.Top
  ...
  given JoinValue[Value] with
    override def joinValues(v1: Value, v2: Value): Value = (v1, v2) match
      ...
      case (DoubleValue(d1), DoubleValue(d2)) => DoubleValue(doubleIntervalJoin.joinValues(d1, d2))
      case _ => TopValue

  given fix.Widening[Value] with
    override def widen(v1: Value, v2: Value): Value = (v1, v2) match
      case (BooleanValue(b1), BooleanValue(b2)) => BooleanValue(flatToppedJoin.joinValues(b1, b2))
      case (DoubleValue(d1), DoubleValue(d2)) => DoubleValue(doubleIntervalWiden.widen(d1, d2))
      case _ => TopValue
  ...
  def apply(initEnvironment: Environment, initStore: Store): IntervalAnalysis = {
    ...
    given DoubleOps[Value] = new LiftedDoubleOps[Value, DoubleInterval](_.asDouble, DoubleValue.apply)
    given CompareOps[Value, Value]  = new LiftedCompareOps[Value, Value, DoubleInterval, Topped[Boolean]](_.asDouble, BooleanValue.apply)
