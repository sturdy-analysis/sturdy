...
import sturdy.effect.callframe.{DecidableCallFrame, JoinableConcreteCallFrame}
...
import sturdy.effect.{AnalysisState, Effectful, store, given}
...
import sturdy.values.abstraction.symbolic.{*, given}
...
object RelationalAnalysis extends Interpreter,
  Ints.Relational, Functions.Powerset, Records.PreciseFieldsOrTop, References.AllocationSites, Fix:
  ...
  class Instance(initEnvironment: Environment, initStore: Store, stackedFrames: Boolean, callSites: Int) extends GenericInstance:
    ...
    override val input: AUserInput[Value] = new AUserInput(Value.IntValue(IntExp.Rand()))
