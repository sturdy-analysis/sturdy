...
import sturdy.values.relational.{EqOps, LiftedOrderingOps, OrderingOps}
import sturdy.values.*

/**
 * Trait [[BackwardsInterpreter]] allows sharing code between concrete and abstract interpreters.
 * The trait defines different types of values and conversions between them.
 */
trait BackwardsInterpreter:
  type VBool
  type VInt
  type VRef
  type VFun
