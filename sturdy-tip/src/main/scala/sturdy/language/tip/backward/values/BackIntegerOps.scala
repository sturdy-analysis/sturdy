...
class LiftedBackIntegerOps[B,V,I](extract: V => I, inject: I => V)(using ops: BackIntegerOps[B,I]) extends BackIntegerOps[B,V]:
  ...
  given SignBackIntegerOps[B](using failure: Failure, j: EffectStack, base: Integral[B]): IntegerOps[B, IntSign => IntSign] with
    override def integerLit(i: B): IntSign => IntSign =
      if base.lt(i, base.zero) then _ => Neg
      else if base.gt(i, base.zero) then _ => Pos
      else _ => Zero
...
//      case (i :: _) => integerLit(i)
    ...
    override def randomInteger(): IntSign => IntSign = _ => TopSign
