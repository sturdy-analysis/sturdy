...
import sturdy.data.{MayJoin, noJoin, CombineUnit}
...
trait GenericBackwardsInterpreter[V, Addr] extends sturdy.Executor:
  ...
  def eval_open(e: Exp)(using Fixed): V = ???

  def evalBack_open(e: Exp, v: Option[V])(using BackFixed): V = e match {
  ...
  def evalBackNonzero(e: Exp, postVal: Option[V])(using BackFixed): V =
    val v = evalBack(e, postVal)
    ...
    case Stm.If(cond: Exp, thn: Stm, els: Option[Stm]) => els match
      ...
      case Some(elsStm) =>
        junit.eff.joinComputations {
          ...
          runBack(elsStm)
          evalBack(cond, Some(integerLit(0)))
          ()
        }
  ...
  def assignBack(lhs: Assignable, s: Stm.Assign)(using Fixed): V = lhs match
    case Assignable.AVar(x) =>
      callFrame.getLocalByName(x).getOrElse(failure(BackwardsUnboundVariable, s.toString))
...
//      store.write(recAddr, updated)
  ...
  inline def evalBack(e: Exp, v: Option[V])(using rec: BackFixed): V = rec(BackFixIn.Eval(e, v)) match {case BackFixOut.Eval(v) => v; case _ => throw new IllegalStateException()}
