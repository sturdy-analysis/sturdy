...
import sturdy.effect.print.{BackwardPrint, Print}
import sturdy.effect.store.{BackwardStore, MustStore, Store}
import sturdy.effect.userinput.{BackwardUserInput, UserInput}
import sturdy.fix
import sturdy.language.tip.TipFailure.UnboundVariable
import sturdy.language.tip.oldBackward.TipBackFailure.*
...
trait GenericBackwardsInterpreter[V, Addr] extends sturdy.Executor:
  ...
  def executeBack(p: Program, expected: V): (Seq[V], V) = external {
    ...
    val test2 = (Seq(intOps.toValue(List(5))), expected)
    test
  }
