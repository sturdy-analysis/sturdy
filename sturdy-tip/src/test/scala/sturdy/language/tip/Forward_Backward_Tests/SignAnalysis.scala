package sturdy.language.tip.Forward_Backward_Tests
...
class SignAnalysisTest extends AnyFlatSpec, Matchers:
  ...
  val uri = classOf[SignAnalysisTest].getResource("/MyTests").toURI;
  ...
  def runSignAnalysis(p: Path, stackConfig: StackConfig) =
    ...
    if (program.funs.exists(_.name == "main")) {
      ...
      println(s"aresult:${aresult}")
      println(s"cresult:${cresult}")
      println(s"interp:${interp}")
      println(s"interp:${analysis}")
      (aresult, analysis)
    } else {
      null
    }
