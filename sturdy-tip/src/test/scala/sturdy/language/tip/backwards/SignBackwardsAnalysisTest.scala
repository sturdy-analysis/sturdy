...
import sturdy.language.tip.analysis.SignAnalysis
...
class SignBackwardsAnalysisTest extends AnyFlatSpec, Matchers:
  ...
  behavior of "BackwardForward SignAnalysis on MyTests"

  Files.list(Paths.get(uriMyTests)).toScala(List).filter(p =>
    p.getFileName.toString != "simple4.tip"
  ).foreach { p =>
    it must s"soundly analyze ${p.getFileName} with stacked states" in {
      runBackwardForwardSignAnalysis(p, StackConfig.StackedStates())
    }
  }
...
def runBackwardForwardSignAnalysis(p: Path, stackConfig: StackConfig) =
  val file = Source.fromURI(p.toUri)
  val sourceCode = file.getLines().mkString("\n")
  file.close()
  val program = Parser.parse(sourceCode)
  ...
  if (functions.contains("main")) {
    ...
    val oldaresult = oldAnalysis.failure.fallible(analysis.executeBack(program, Value.TopValue))
    val aresult = analysis.failure.fallible(analysis.executeBack(program, Value.TopValue))


    val inputs = () => ConcreteInterpreter.Value.IntValue(0)
    val forwardAnalysis = new SignAnalysis.Instance(Map(), Map(), stackConfig)
    val faresult = forwardAnalysis.failure.fallible(forwardAnalysis.execute(program))

    println("LOOOK HERE _----------------------- " + faresult)

//    val res = faresult.get.asInt(using forwardAnalysis.failure)
//    val expectedVal = analysis.intOps.toValue(List(res))
