...
def getCfgDifference(oldCfg: ControlFlowGraph[CfgNode, _], newCfg: ControlGraph[InstLoc, FuncId | InstLoc]):
  (Set[(Node[InstLoc, FuncId | InstLoc], Node[InstLoc, FuncId | InstLoc])], Set[(Node[InstLoc, FuncId | InstLoc], Node[InstLoc, FuncId | InstLoc])]) = {
  type OldNode = CfgNode
  val oldNodes: Set[OldNode] = oldCfg.getNodes.map(_.node).toSet
  val oldEdges: Set[(OldNode, OldNode)] = oldCfg.getEdgesFlat.map(e => e._1.node -> e._2.node).toSet
