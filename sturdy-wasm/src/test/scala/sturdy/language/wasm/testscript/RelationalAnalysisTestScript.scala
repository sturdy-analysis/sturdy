  ...
  Files.list(Paths.get(uri)).toScala(List).filter(p => p.toString.endsWith(".wast")).sorted.foreach { p =>
    for (analysis <- analyses) {
      ...
      it must s"execute ${p.getFileName} with ${anl} and ${anl.apronManager}" in {
        ...
        val interp = RelationalAnalysisTestScriptInterpreter(Some(spectest), anl)
        interp.run(script)
//        val interpTop = RelationalAnalysisTestScriptInterpreter(Some(spectest), aInterp(), true)
...
//      aInterp.recencyStore.garabageCollect()
  ...
  def constExprToTop(inst: unresolved.Inst): RelationalAnalysis.Value =
    inst match
      case unresolved.i32.Const(_) => RelationalAnalysis.Value.Int32(RelationalAnalysis.topI32)
      case unresolved.i64.Const(_) => RelationalAnalysis.Value.Int64(RelationalAnalysis.topI64)
      case unresolved.f32.Const(_) => RelationalAnalysis.Value.Float32(RelationalAnalysis.topF32)
      case unresolved.f64.Const(_) => RelationalAnalysis.Value.Float64(RelationalAnalysis.topF64)
      case _ => throw IllegalArgumentException(s"Expected constant instruction but got $inst")
