...
import sturdy.language.wasm.analyses.FloatingIntervalAnalysisSoundness.given
import sturdy.language.wasm.analyses.{FixpointConfig, FloatingIntervalAnalysis, Insensitive, WasmConfig}
...
class FloatingIntervalAnalysisTestScript extends AnyFlatSpec, Matchers:
  ...
  def analyses: IterableOnce[() => FloatingIntervalAnalysis.Instance] =
    Iterator(
      () => new FloatingIntervalAnalysis.Instance(FrameData.empty, Iterable.empty, WasmConfig(fix = FixpointConfig(iter = fix.iter.Config.Innermost(StackConfig.StackedStates(readPriorOutput = false))), ctx = Insensitive)),
  ...
  Files.list(Paths.get(uri)).toScala(List).filter(p => p.toString.endsWith(".wast") && !p.toString.endsWith("memory_grow.wast") ).sorted.foreach { p =>
    for (aInterp <- analyses) {
      it must s"execute ${p.getFileName} with ${aInterp()}" in {
        ...
        val interp = FloatingIntervalAnalysisTestScriptInterpreter(Some(spectest), aInterp())
        interp.run(script)
        val interpTop = FloatingIntervalAnalysisTestScriptInterpreter(Some(spectest), aInterp(), true)
...
class FloatingIntervalAnalysisTestScriptInterpreter(spectest: Option[Module] = None, aInterp: FloatingIntervalAnalysis.Instance, useTop: Boolean = false):
  type CValue = ConcreteInterpreter.Value
  type AValue = FloatingIntervalAnalysis.Value
  ...
  val convertVals: unresolved.Expr => List[FloatingIntervalAnalysis.Value] =
    if (useTop)
      constExprToTops
    else
      constExprToAVals
  ...
  def runAAction(a: Action, convertVals: unresolved.Expr => List[FloatingIntervalAnalysis.Value]): AResult = a match {
  ...
  def constExprToAVals(e: unresolved.Expr): List[FloatingIntervalAnalysis.Value] =
    e.map(constExprToAVal).toList

  def constExprToAVal(inst: unresolved.Inst): FloatingIntervalAnalysis.Value = Abstractly(constExprToVal(inst))

  def constExprToTops(e: unresolved.Expr): List[FloatingIntervalAnalysis.Value] =
    e.map(constExprToTop).toList

  def constExprToTop(inst: unresolved.Inst): FloatingIntervalAnalysis.Value =
    inst match
      case unresolved.i32.Const(_) => FloatingIntervalAnalysis.Value.Int32(Top.top)
      case unresolved.i64.Const(_) => FloatingIntervalAnalysis.Value.Int64(Top.top)
      case unresolved.f32.Const(_) => FloatingIntervalAnalysis.Value.Float32(Top.top)
      case unresolved.f64.Const(_) => FloatingIntervalAnalysis.Value.Float64(Top.top)
      case _ => throw IllegalArgumentException(s"Expected constant instruction but got $inst")
