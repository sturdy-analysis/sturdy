...
class ConstantAnalysisKeidelTestScript extends AnyFlatSpec, Matchers:
  behavior of "TestScript constant analysis keidel"
  ...
  Files.list(Paths.get(uri)).toScala(List).filter(p => p.toString.endsWith(".wast")).sorted.foreach { p =>
    it must s"execute ${p.getFileName}" in {
      ...
      val interp = ConstantAnalysisKeidelTestScriptInterpreter(Some(spectest))
      interp.run(script)
      val interpTop = ConstantAnalysisKeidelTestScriptInterpreter(Some(spectest), true)
      interpTop.run(script)
    }
  }


class ConstantAnalysisKeidelTestScriptInterpreter(spectest: Option[Module] = None, useTop: Boolean = false):
  ...
  val aInterp = new ConstantAnalysis.Instance(FrameData.empty, Iterable.empty) {

    override val fixpointSuper: fix.Fixpoint[FixIn, FixOut[AValue]] = new KeidelFixpoint(
    ...
    dummy = dummy.appended(ConstantAnalysis.Value.TopValue)
  }
