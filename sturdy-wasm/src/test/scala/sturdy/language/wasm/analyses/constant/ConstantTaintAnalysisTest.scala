...
class ConstantTaintAnalysisTest extends AnyFlatSpec, Matchers:
  ...
  def testFunction(path: Path, funcName: String, args: List[ConstantAnalysis.Value], expected: List[ConstantAnalysis.Value]) =
    it must s"execute $funcName withs args $args with result $expected" in {
      val res = runConstantTaintAnalysis(path, funcName, args.map(ConstantTaintAnalysis.liftConstantValue(_,true)))
      res match
        case AFallible.Unfailing(vals) => assertResult(expected)(vals.map(untaint))
        case AFallible.MaybeFailing(vals, _) => assertResult(expected)(vals.map(untaint))
