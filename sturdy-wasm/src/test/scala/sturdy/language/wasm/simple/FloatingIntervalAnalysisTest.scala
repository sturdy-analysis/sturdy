...
import sturdy.language.wasm.analyses.FloatingIntervalAnalysis.Value
import sturdy.language.wasm.analyses.{FixpointConfig, FloatingIntervalAnalysis, WasmConfig}
import sturdy.language.wasm.generic.{FrameData, WasmFailure}
import sturdy.util.{LinearStateOperationCounter, Profiler}
import sturdy.values.Abstractly
import sturdy.values.floating.FloatingInterval
import sturdy.values.integer.{IntegerDivisionByZero, NumericInterval}

import java.nio.file.{Path, Paths}

class FloatingIntervalAnalysisTest extends AnyFlatSpec, Matchers:
...

  {
    ...
    testFailingFunction(simple, "division",
      List(FloatingIntervalAnalysis.Value.Int32(NumericInterval.constant(1)),
      FloatingIntervalAnalysis.Value.Int32(NumericInterval.constant(0))),
  ...
  val topI32 = Value.Int32(FloatingIntervalAnalysis.topI32)
  val topI64 = Value.Int64(FloatingIntervalAnalysis.topI64)
  val topF32 = Value.Float32(FloatingIntervalAnalysis.topF32)
  val topF64 = Value.Float64(FloatingIntervalAnalysis.topF64)

  testFunction(simple, "const", List(topI32), List(topI32))
  testFunction(simple, "first", List(Value.Int32(NumericInterval.constant(1)), topI32), List(Value.Int32(NumericInterval.constant(1))))
  testFunction(simple, "first", List(topI32, Value.Int32(NumericInterval.constant(2))), List(topI32))
  testFunction(simple, "second", List(Value.Int32(NumericInterval.constant(1)), topI32), List(topI32))
  testFunction(simple, "second", List(topI32, Value.Int32(NumericInterval.constant(2))), List(Value.Int32(NumericInterval.constant(2))))
  testFunction(simple, "test-mem0", List(topI32), List(topI32))
  testFunction(simple, "test-mem", List(topI32), List(topI32))
  testFunction(simple, "nesting", List(topF32, topF32), List(topF32))
  testFunction(simple, "nesting", List(Value.Float32(FloatingInterval.constant(1)), topF32), List(topF32))
  testFunction(simple, "test-br3", List(topI32), List(Value.Int32(NumericInterval(42, 43))))
  testFunction(simple, "test-br-and-return", List(topI32), List(Value.Int32(NumericInterval(42, 43))))
  testFunction(simple, "test-br-and-return2", List(topI32), List(Value.Int32(NumericInterval(42, 43))))
  testFunction(simple, "test-br-and-return3", List(topI32), List(Value.Int32(NumericInterval(42, 43))))
  testFunction(simple, "test-br-and-return4", List(topI32), List(Value.Int32(NumericInterval.constant(42))))
  testFunction(simple, "test-unreachable5", List(topI32), List(Value.Int32(NumericInterval(42, 43))))
  testFunction(simple, "test-global", List(topI32), List(Value.Int32(NumericInterval(1, 2))))
  testFunction(simple, "test-call-indirect-parametric", List(topI32), List(Value.Int32(NumericInterval.constant(0))))
  testFailingFunction(simple, "division", List(Value.Int32(NumericInterval.constant(1)), topI32), IntegerDivisionByZero)
  testFunction(simple, "effects", List(topI32), List(Value.Int32(NumericInterval(-14, -6))))

  //  testFunction(fact, "fac-rec", List(Value.Int64(NumericInterval.constant(1))), List(Value.Int64(NumericInterval.constant(1))))
  (2 to 8).foreach { arg =>
    testFunction(fact, "fac-rec", List(Value.Int64(NumericInterval.constant(arg))), List(topI64))
  }
  testFunction(fact, "fac-rec", List(Value.Int64(NumericInterval.constant(25))), List(topI64))
  testFunction(fact, "fac-iter", List(Value.Int64(NumericInterval.constant(25))), List(topI64))
  testFunction(fact, "fac-rec-named", List(Value.Int64(NumericInterval.constant(25))), List(topI64))
  testFunction(fact, "fac-iter-named", List(Value.Int64(NumericInterval.constant(25))), List(topI64))
  testFunction(fact, "fac-opt", List(Value.Int64(NumericInterval.constant(25))), List(topI64))
  ...
  testFunction(simpleTest, "main", List(topI32), List(Value.Int32(NumericInterval(42, 42))))
  ...
  def testFunction(path: Path, funcName: String, args: List[Value], expected: List[Value]) =
    it must s"execute $funcName withs args $args with result $expected with stacked states" in {
      val res = runFloatingIntervalAnalysis(path, funcName, args, StackConfig.StackedStates())
      res match
...
def runFloatingIntervalAnalysis(path: Path, funName: String, args: List[Value], stackConfig: StackConfig): AFallible[List[Value]] =
  val module = wasm.Parsing.fromText(path)

  val interp = new FloatingIntervalAnalysis.Instance(FrameData.empty, Iterable.empty,
    WasmConfig(FixpointConfig(fix.iter.Config.Innermost(stackConfig))))
  val constants = FloatingIntervalAnalysis.constantInstructions(interp)
