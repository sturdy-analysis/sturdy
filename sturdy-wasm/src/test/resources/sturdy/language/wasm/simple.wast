(module
  ...
  (func (export "operand_stack_is_immutable") (result i32) (local $x i32)
                  (; Concrete Interpretation   Relational Analysis ;)
                  (; Stack, Locals             Stack,       Locals               AddrTrans                  Store                   ;)
    i32.const 0   (; [0]    $x undefinded      [0]          $x_recent undefined                                                     ;)
    local.set $x  (; []     $x = 0             []           $x = x1              x1 = x_recent              x_recent = 0            ;)
    local.get $x  (; [0]    $x = 0             [x1]         $x = x1              x1 = x_recent              x_recent = 0            ;)
    i32.const 1   (; [1,0]  $x = 0             [1,x1]       $x = x1              x1 = x_recent              x_recent = 0            ;)
    local.set $x  (; [0]    $x = 1             [x1]         $x = x2              x1 = x_old, x2 = x_recent  x_recent = 1, x_old = 0 ;)
    return        (; 0 is returned             x1 is returned                                                                       ;)
  )

  (func (export "plus_five") (param $x i32) (result i32)
      ...
      (return (local.get $x))                               (; x ∈ [5,105]                           xr ∈ [1,105]              ;)
    )

  (func (export "loop_to_100") (param $x i32) (result i32)
    (block $exit
        (loop $continue
