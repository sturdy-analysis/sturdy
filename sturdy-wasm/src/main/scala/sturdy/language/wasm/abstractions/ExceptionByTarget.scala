...
import sturdy.language.wasm.generic.{FixIn, FixOut, JumpTarget, WasmException}
...
trait ExceptionByTarget extends Interpreter:
  ...
  given JoinExceptionState(using lazyEffectStack: Lazy[EffectStack], joinValue: Join[Value]): Join[ExceptionState] =
    (state1, state2) =>
      lazyEffectStack.value.joinOut[List[Value]](FixIn.Exception)((state1.operands, state1.effectState), (state2.operands, state2.effectState)).map(ExceptionState.apply)

  given WidenExceptionState(using lazyEffectStack: Lazy[EffectStack], widenValue: Widen[Value]): Widen[ExceptionState] =
    (state1, state2) =>
      lazyEffectStack.value.widenOut[List[Value]](FixIn.Exception)((state1.operands, state1.effectState), (state2.operands, state2.effectState)).map(ExceptionState.apply)
