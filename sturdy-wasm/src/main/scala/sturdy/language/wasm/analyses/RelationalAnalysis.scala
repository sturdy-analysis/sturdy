...
object RelationalAnalysis extends Interpreter, RelationalValues, ExceptionByTarget, ControlFlow, Control:
  type J[A] = WithJoin[A]
  type Addr = I32
  ...
  given RelationalSpecialWasmOperations(using f: Failure, eff: EffectStack): SpecialWasmOperations[Value, Addr, Size, FuncIx, WithJoin] with
...
//            JOptionPowerset.NoneSome(Powerset(vec.toSet))
  ...
  class Instance(rootFrameData: FrameData, rootFrameValues: Iterable[Value], config: WasmConfig) extends
    ...
    private given ApronState[VAddr, Type] = apronState
