...
import FloatingIntervalAnalysis.*
...
object FloatingIntervalAnalysisSoundness {
  ...
  given po: PartialOrder[Value] with
    override def lteq(x: Value, y: Value): Boolean = (x,y) match
      ...
      case (Value.Float32(f1), Value.Float32(f2)) => PartialOrder[FloatingInterval[Float]].lteq(f1,f2)
      case (Value.Float64(d1), Value.Float64(d2)) => PartialOrder[FloatingInterval[Double]].lteq(d1,d2)
      case _ => false
  ...
  given Soundness[ConcreteInterpreter.Instance, FloatingIntervalAnalysis.Instance] with
    def isSound(c: ConcreteInterpreter.Instance, a: FloatingIntervalAnalysis.Instance): IsSound =
