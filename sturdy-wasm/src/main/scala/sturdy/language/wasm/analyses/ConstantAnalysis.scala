...
object ConstantAnalysis extends Interpreter, ConstantValues, ControlFlow:
  ...
  given ConstantSpecialWasmOperations(using f: Failure, eff: EffectStack): SpecialWasmOperations[Value, Addr, Size, FuncIx, WithJoin] with
    ...
    override def invokeHostFunction(hostFunc: HostFunction, args: List[ConstantAnalysis.Value]): List[ConstantAnalysis.Value] = hostFunc match
      ...
      case HostFunction.args_get => eff.joinWithFailure(List(Value.Int32(Topped.Top)))(f.fail(InvocationError, s"in ${hostFunc.name}"))
      case HostFunction.environ_sizes_get => List(Value.Int32(Topped.Top))
      case HostFunction.environ_get => eff.joinWithFailure(List(Value.Int32(Topped.Top)))(f.fail(InvocationError, s"in ${hostFunc.name}"))
      case HostFunction.fd_prestat_get => eff.joinWithFailure(List(Value.Int32(Topped.Top)))(f.fail(FileError, s"in ${hostFunc.name}"))
      case HostFunction.random_get => List(Value.Int32(Topped.Top))
      case HostFunction.path_open => eff.joinWithFailure(List(Value.Int32(Topped.Top)))(f.fail(FileError, s"in ${hostFunc.name}"))
      case HostFunction.fd_prestat_dir_name => eff.joinWithFailure(List(Value.Int32(Topped.Top)))(f.fail(FileError, s"in ${hostFunc.name}"))
