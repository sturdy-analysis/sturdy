...
object FloatingIntervalAnalysis extends Interpreter, FloatingIntervalValues, ExceptionByTarget, ControlFlow, Control:
  type J[A] = WithJoin[A]
  type Addr = I32
  ...
  given ConstantSpecialWasmOperations(using f: Failure, eff: EffectStack): SpecialWasmOperations[Value, Addr, Size, FuncIx, WithJoin] with
    ...
    override def invokeHostFunction(hostFunc: HostFunction, args: List[FloatingIntervalAnalysis.Value]): List[FloatingIntervalAnalysis.Value] = hostFunc.name match
      case "proc_exit" =>
  ...
  given valuesAbstractly: Abstractly[ConcreteInterpreter.Value, Value] with
    override def apply(c: ConcreteInterpreter.Value): Value = c match
      ...
      case ConcreteInterpreter.Value.Float32(f) => Value.Float32(FloatingInterval.constant(f))
      case ConcreteInterpreter.Value.Float64(d) => Value.Float64(FloatingInterval.constant(d))
  ...
  class Instance(rootFrameData: FrameData, rootFrameValues: Iterable[Value], config: WasmConfig) extends
    ...
    var floatBounds: Set[Float] = Set(-1f, 0, 1f)
    var doubleBounds: Set[Double] = Set(-1d, 0, 1d)
    ...
    given Widen[F32] = new FloatingIntervalWiden[Float](floatBounds, Float.NegativeInfinity,Float.PositiveInfinity)
    given Widen[F64] = new FloatingIntervalWiden[Double](doubleBounds, Double.NegativeInfinity, Double.PositiveInfinity)
    ...
    override val fixpoint: fix.ContextualFixpoint[FixIn, FixOut[FloatingIntervalAnalysis.Value]] = new fix.ContextualFixpoint {
      override type Ctx = observedConfig.ctx.Ctx
      val (contextPreparation, sensitivity) = observedConfig.ctx.make[FloatingIntervalAnalysis.Value]
