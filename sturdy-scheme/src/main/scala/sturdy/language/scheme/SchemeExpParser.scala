...
object SchemeExpParser:
  ...
  def parse(source: String): List[Exp] = SExpParser.parse(source).map(transform)

  def transform(se: SExp): Exp = se match
    case sexpr(es) => transformExprs(es)
    case nonsexpr => transformNonSExpr(nonsexpr)

  def transformNonSExpr(se: SExp): Exp = se match
  ...
  def transformBody(ses: SExp): Body = ???

  def transformExprs(ses: List[SExp]): Exp = ses match
    // Nil
    // Cons
    // Begin
    // Var
...
//    case List(id("define"), sexpr(id(name) :: args), body) => Define(name, Lam(args.map(liftVar), List(transform(body))))
    ...
    case id("let") :: sexpr(args) :: body =>
      ...
      Let(argsE, transformBody(sexpr(body)))
    // LetRec
    // Op1, missing: random
    ...
    // App
    case id(procVar) :: args => Apply(Var(procVar), args.map(transform))
