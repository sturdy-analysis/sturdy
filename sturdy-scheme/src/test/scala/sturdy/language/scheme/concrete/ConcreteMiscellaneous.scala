...
class ConcreteMiscellaneous extends AnyFlatSpec, Matchers:

  behavior of "Scheme miscellaneous tests"

  it should "run (fac 10)" in {
    val res = Utils().runProgram(Examples.factorial)
    assertResult(NumVal(IntVal(3628800)))(res)
  }

  it should "test set! var" in {
    val res = Utils().runAny(List(
